...
if(isset($_POST['submit'])){
	...
	if(strlen($_POST['username']) < 3){
		...
		$stmt = $db->prepare('SELECT nomuser FROM user WHERE nomuser = :username');
		$stmt->execute(array(':username' => $_POST['username']));
		$row = $stmt->fetch(PDO::FETCH_ASSOC);

		if(!empty($row['nomuser'])){
	...
	if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
		...
		$stmt = $db->prepare('SELECT adressemail FROM user WHERE adressemail = :email');
		$stmt->execute(array(':email' => $_POST['email']));
		$row = $stmt->fetch(PDO::FETCH_ASSOC);

		if(!empty($row['adressemail'])){
	...
	//	$activasion = md5(uniqid(rand(),true));
		...
		try {
			...
			$stmt = $db->prepare('INSERT INTO user ( nom, prenom, nomuser,motpasse,adressemail,active , numtel) VALUES (:nom , :prenom, :username, :password, :email, :active , :numtel)');
			$stmt->execute(array(
				':nom' => $_POST['nom'],
				':prenom' => $_POST['prenom'],
			...
			$mail->SMTPAuth = true;                               
			$mail->Username = 'ds_boukhetta@esi.dz';                            
			$mail->Password = 'SALAH:/-_-1994';                        
			$mail->SMTPSecure = 'tls';  
			$mail->Port  =  25;
			$mail->From = SITEEMAIL;
...
<h1>Budgeto</h1>
		<div class="login"  style="margin-top:-25px;">
			<form role="form" method="post" action="" autocomplete="off">
				...
				<div class="dumy">
					<li class="psw">
						<a  class=" icon user"></a><input type="text" value="Nom" name="nom" onfocus="this.value = '';this.type='text';" onblur="if (this.value == '') {this.value = 'Nom';this.type='text';}">
					</li>
					<li class="psw two">
						<a  class=" icon user"></a><input type="text" value="Prenom" name="prenom" onfocus="this.value = '';this.type='text';" onblur="if (this.value == '') {this.value = 'Confirmation';this.type='text';}">
					</li>
					<div class="clear"></div>
				</div>
					<li>
					...
					<li>
						<a  class=" icon phone"></a><input type="text" class="text" value="Numero de Telephone" name="numtel" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Numero de Telephone';}" >
					</li>
				<div class="dumy">
					<li class="psw">
					...
					<div class="submit">
							...
							<?php
								...
								if(isset($_GET['action']) && $_GET['action'] == 'joined'){
									echo "<h2 class='bg-success'>Inscription réussi.</h2>";
								}
							?>
			      	...
			      	<div class="dropbox"> <a class="drop" href="login.php" style="text-decoration:none;">Connexion</a>
			      	</div>
			     <label><a href="../../accueil/">Retoure à la page d'accueil </a></label>
