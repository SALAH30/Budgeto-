...
if(strcmp($debut,$fin)>0){
    echo "Erreur dans la date ( date fin avant date debut !! )";
            ...
            $statement = $db->prepare('INSERT
             INTO Colloque ( nomCol , dateColDebut , dateColFin , lieuCol , descCol , droitCol , nbParticipants , idUser )
                                          
                       VALUES ( :nomCol , :dateColDebut , :dateColFin , :lieuCol , :descCol  , :droitCol , :nbParticipants , :iduser)');
                if(isset($_POST['nom']))
                {
                    $clean['nom'] = $_POST['nom'];  
                ...
                if(isset($_POST['nom']))
                {
                     try{
                               ...
                               $statement->execute();
echo "Colloque : ".$_POST['nom']." inséré avec succés";
                    }catch (Exception $e) {
                            echo "non inséré";
                            exit($e ->getMessage());
